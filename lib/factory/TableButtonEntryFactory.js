...
var TABLE_ROW_DIV_SNIPPET = '<div class="bpp-field-wrapper bpp-table-row my-vertical-div">';
...
function createButtonRowTemplate(infos) {
  var template = TABLE_ROW_DIV_SNIPPET;
  template += createButtonTemplate(infos);
  template += '</div>';

  return template;
}

function createButtonTemplate(infos) {
  var columns = infos.length;
  var template = '';
  forEach(infos, function(info) {
    template += '<button type="button" class="my-button" data-entry="activities" data-item="'+ info.xml + '" data-action="linkSelected">' + info.name + '</label>';
  });
  return template;
}
...
module.exports = function(options) {
  ...
  var labelRow = createButtonRowTemplate(infos);
  ...
  var factory = {
    ...
    linkSelected: function(element, node, event, scopeNode, inputNode) {
      if (typeof getClickableElement === 'function') {
        getClickableElement(element, node, event, scopeNode, inputNode);
        // var link = getClickableElement.apply();
