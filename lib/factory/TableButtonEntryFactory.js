...
function createButtonRowTemplate(labels) {
  var template = TABLE_ROW_DIV_SNIPPET;
  template += createButtonTemplate(labels);
  template += '</div>';
...
function createButtonTemplate(labels) {
  ...
  forEach(labels, function(label) {
    template += '<button type="button" class="my-button" data-action="linkSelected">' + label + '</label>';
  });
  return template;
}
...
module.exports = function(options) {
  ...
  var labelRow = createButtonRowTemplate(labels);
