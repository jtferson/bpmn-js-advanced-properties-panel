...
PropertiesPanel.prototype._bindListeners = function(container) {
  ...
  domDelegate.bind(container, '[data-action]', 'click', function onClick(event) {
    ...
    var isEntryDirty = self.executeAction(entry, entryNode, actionId, event, inputNode);
...
PropertiesPanel.prototype.executeAction = function(entry, entryNode, actionId, event, inputNode) {
  var current = this._current;
  ...
  if (fn) {
    ...
    return fn.apply(entry, [ current.element, entryNode, event, scopeNode, inputNode ]);
  }
};
